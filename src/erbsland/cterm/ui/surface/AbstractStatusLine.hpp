...
using AbstractStatusLinePtr = std::shared_ptr<AbstractStatusLine>;
...
class AbstractStatusLine : public Panel {
public:
    /// The available logical sections of the status line.
...
public:
    /// Create a status-line surface with fixed height `1`.
    explicit AbstractStatusLine(ProtectedTag) noexcept;
    ~AbstractStatusLine() override = default;
...
public:
    ...
    /// Only left and right margins affect layout because the status line height is fixed to one row.
...
protected:
    ...
    /// @param totalWidth The available width of the status line.
    ...
    /// Negative margins are clamped to zero for status-line layout.
...
private:
    ...
    /// Draw one section clipped to the target rectangle.
    /// @param buffer The destination buffer.
    /// @param targetRect The target rectangle.
    /// @param x The left-most drawing coordinate.
    /// @param text The text to draw.
    static void
    drawClippedText(WritableBuffer &buffer, Rectangle targetRect, Coordinate x, const StringView &text) noexcept;
