...
protected:
    [[nodiscard]] auto readImpl(const std::chrono::milliseconds timeout) const -> Key override {
...
public:
    ...
    [[nodiscard]] auto get(const Position pos) const noexcept -> const Char & override { return _buffer.get(pos); }
    [[nodiscard]] auto clone() const -> WritableBufferPtr override { return std::make_shared<Buffer>(_buffer); }
    void resize(const Size newSize) override { _buffer.resize(newSize); }
...
public:
    void testInputReadDelegatesToReadImpl() {
        auto input = InputDispatchProbe{};

        REQUIRE_EQUAL(input.read(), Key{Key::Escape});
        REQUIRE_EQUAL(input._lastTimeout, std::chrono::milliseconds{0});

        REQUIRE_EQUAL(input.read(std::chrono::milliseconds{125}), Key{Key::Escape});
