...
#include "HtmlRendererTestSupport.hpp"
...
public:
    ...
    void testRenderStringAppliesConfiguredInlinePrefixesAndSuffixes() {
        auto style = std::make_shared<text::Style>();
        style->edit(text::StyleSelector::strong())
            .setTextStyle(CharStyle{CharAttributes::Bold})
            .setPrefix(U"[")
            .setSuffix(U"]");
        style->edit(text::StyleSelector{text::StyleRole::Span, {"warning"}})
            .setTextStyle(CharStyle{fg::BrightRed})
            .setPrefix(U"<")
            .setSuffix(U">");

        const auto rendered =
            text::HtmlRenderer{"<p>Hello <strong><span class=\"warning\">Alert</span></strong>.</p>", style}
                .renderString();

        REQUIRE_EQUAL(toPlainText(rendered), std::string{"Hello [<Alert>]."});
        REQUIRE(rendered[6].attributes().isBold());
        REQUIRE(rendered[7].attributes().isBold());
        REQUIRE_EQUAL(rendered[7].color().fg(), fg::BrightRed);
        REQUIRE(rendered[13].attributes().isBold());
        REQUIRE_EQUAL(rendered[13].color().fg(), fg::BrightRed);
        REQUIRE(rendered[14].attributes().isBold());
        REQUIRE_NOT_EQUAL(rendered[0].color().fg(), fg::BrightRed);
    }

    void testRenderStringTrimsNestedLeadingAndTrailingInlineWhitespace() {
        const auto rendered =
            text::HtmlRenderer{"<p> <span><strong>Hello</strong></span><span> </span><em>world</em> </p>"
                               "<p><span> </span><span><strong>Trimmed</strong></span><span> </span></p>"}
                .renderString();
        const auto expected = std::vector<std::string>{"Hello world", "Trimmed"};
    ...
    void testRenderToCollapsesNestedContainerMarginsIntoFirstAndLastBlocks() {
        auto style = std::make_shared<text::Style>();
        style->edit(text::StyleSelector::document()).setMargins(2, 0, 4, 0);
        style->edit(text::StyleSelector::section()).setMargins(1, 0, 3, 0);
        style->edit(text::StyleSelector::paragraph()).setMargins(3, 0, 2, 0);

        auto buffer = std::make_shared<CursorBuffer>(Size{8, 10});
        text::HtmlRenderer{"<section><p>Text</p></section>", style}.renderTo(buffer);
    ...
    void testRenderToAppliesConfiguredInlinePrefixesAndSuffixes() {
        auto style = std::make_shared<text::Style>();
        style->edit(text::StyleSelector::strong()).setPrefix(U"(").setSuffix(U")");

        auto buffer = std::make_shared<CursorBuffer>(Size{6, 2});
        text::HtmlRenderer{"<p><strong>AB</strong></p>", style}.renderTo(buffer);
