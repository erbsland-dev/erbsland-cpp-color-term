...
public:
    ...
    void testParseConsoleInputPrefixSupportsUnicodeAndCombinedCharacters() {
        const auto unicode = term::impl::KeyDecoder{bytes({0xC3, 0xA4, 0x2B})}.parseConsoleInputPrefix();
        ...
        const auto combined = term::impl::KeyDecoder{bytes({0x65, 0xCC, 0x81, 0x78})}.parseConsoleInputPrefix();
    ...
    void testDecodeConsoleInputRejectsIncompleteOrMultiKeyInput() {
        REQUIRE_EQUAL(term::impl::KeyDecoder{bytes({0x65, 0xCC})}.decodeConsoleInput(), Key{Key::None});
    ...
    void testParseConsoleInputPrefixResynchronizesAfterBrokenUtf8LeadByte() {
        const auto brokenThenAscii = term::impl::KeyDecoder{bytes({0xC3, 0x71})}.parseConsoleInputPrefix();
