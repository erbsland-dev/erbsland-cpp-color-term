...
public:
    ...
    void testDrawFrameWithNoneStyleDrawsColoredSpacesOnThePerimeter() {
        Buffer buffer(Size(3, 3));
        buffer.fill(Char{U'X', fg::Green, bg::Blue});

        buffer.drawFrame(Rectangle{0, 0, 3, 3}, FrameStyle::None, Color{fg::Red, bg::Black});

        requireRowsEqual(buffer, {"   ", " X ", "   "});
        REQUIRE_EQUAL(buffer.get(Position{0, 0}).color(), Color(fg::Red, bg::Black));
        REQUIRE_EQUAL(buffer.get(Position{1, 1}).color(), Color(fg::Green, bg::Blue));
    }
    ...
    void testDrawGridLayoutWithDefaultBorderDrawsNothing() {
        Buffer buffer(Size(5, 3));
        buffer.fill(Char{U'.'});

        buffer.drawGridLayout(Position{}, GridLayout{{2, 3}, {1, 2}}, FrameBorder{});

        requireRowsEqual(buffer, {".....", ".....", "....."});
    }

    void testDrawGridLayoutWithLightBorderRendersATable() {
        Buffer buffer(Size(19, 7));

        buffer.drawGridLayout(Position{}, GridLayout{{5, 5, 5}, {1, 1, 1}}, FrameBorder{FrameStyle::Light});
    ...
    void testDrawGridLayoutPreservesDashedLineSegments() {
        Buffer buffer(Size(7, 3));

        buffer.drawGridLayout(Position{}, GridLayout{{2, 2}, {1}}, FrameBorder{FrameStyle::LightDoubleDash});
    ...
    void testDrawGridLayoutUsesStraightSegmentsAtMissingOuterCorners() {
        {
            Buffer buffer(Size(7, 4));
            auto border = FrameBorder{};
            border.set(FrameBorder::Element::Top, FrameStyle::Light);

            buffer.drawGridLayout(Position{}, GridLayout{{7}, {3}}, border);
        ...
        }
        {
            ...
            buffer.drawGridLayout(Position{}, GridLayout{{2}, {3}}, border);
    ...
    void testDrawGridLayoutCombinesSeparatorCrossings() {
        ...
        buffer.drawGridLayout(Position{}, GridLayout{{2, 2}, {1, 1}}, border);

        requireRowsEqual(buffer, {"  │  ", "──┼──", "  │  "});
    }

    void testDrawGridLayoutDrawsRoundedOuterCornersWithSquareInternalJoins() {
        Buffer buffer(Size(7, 5));

        buffer.drawGridLayout(Position{}, GridLayout{{2, 2}, {1, 1}}, FrameBorder{FrameStyle::LightWithRoundedCorners});
    ...
    void testDrawGridLayoutUsesSquareCornerForMixedRoundedAndLightStyles() {
        ...
        buffer.drawGridLayout(Position{}, GridLayout{{3}, {2}}, border);
    ...
    void testDrawGridLayoutAppliesElementColors() {
        ...
        REQUIRE_EQUAL(buffer.get(Position{2, 1}).color(), Color(fg::Red, bg::Black));
    }
