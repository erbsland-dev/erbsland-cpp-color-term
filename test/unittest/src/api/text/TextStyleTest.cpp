...
#include "TextStyleTestSupport.hpp"

#include <erbsland/unittest/UnitTest.hpp>

TESTED_TARGETS(Style StyleSelector StyleRule StyleMarker)
class TextStyleTest final : public UNITTEST_SUBCLASS(TextStyleTestSupport) {
public:
    ...
    void testStyleSelectorSpanFactorySupportsTokenSpecificSelectors() {
        const auto selector = text::StyleSelector::span({"featured", "warning"});

        REQUIRE_EQUAL(selector.styleRole(), text::StyleRole::Span);
        REQUIRE_EQUAL(selector.listKind(), text::StyleListKind::Any);
        REQUIRE_EQUAL(selector.requiredStyleTokens(), std::vector<std::string>({"featured", "warning"}));
    }
